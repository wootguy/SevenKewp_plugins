...
{
    EHANDLEm_pController;
    EHANDLEm_pControlEntity;
    ...
    int m_classify;
    string m_iszMaster;    // Master entity (game_team_master || multisource)
    ...
    void Spawn()
    {
        ...
        SET_MODEL( self, string(pev.model) );
    ...
    bool StartControl( CBasePlayer* pController )
    {
        ...
        *pev->owner = pController->edict();
        CBaseEntity* pControllEntity = g_EntityFuncs.FindEntityByString(pControllEntity, "target", pev->targetname);
    ...
    CBaseEntity* FindClosestEnemy( string szClassname, float fRadius)
    {
        ...
        do
        {
            *ent = g_EntityFuncs.FindEntityInSphere( ent, pev->origin,
    ...
    void FireEffect( const Vector barrelEnd, const Vector forward, entvars_t* pevAttacker )
    {
        if ( m_fireLast != 0 )
        {
            ...
            SUB_UseTargets( self, USE_TOGGLE, 0 );
        }
    ...
    void StartRotSound()
    {
        ...
        g_SoundSystem.EmitSound( edict(), CHAN_STATIC, string(m_iszRotateSound), 0.85, ATTN_NORM);
    }
    void StopRotSound()
    {
        if ( pev.spawnflags & SF_TANK_SOUNDON != 0 && m_iszRotateSound != "" )
            g_SoundSystem.StopSound( edict(), CHAN_STATIC, string(m_iszRotateSound) );
        pev.spawnflags &= ~SF_TANK_SOUNDON;
    }
}
...
class CFuncTankLaser : CFuncTank
{
    ...
    CLaser* GetLaser()
    {
        ...
        *pentLaser = g_EntityFuncs.FindEntityByTargetname( NULL, string(pev.message) );
        ...
        {
            // Found the landmark
            if ( g_EntityFuncs.FindEntityByClassname( pentLaser, "env_laser" )  )
            {
    ...
    void Fire( const Vector barrelEnd, const Vector forward, entvars_t* pevAttacker )
    {
        if ( m_fireLast != 0 )
        {
            ...
            if ( bulletCount > 0 )
            {
                ...
                Math.MakeAimVectors(pev.angles);
                TankTrace( barrelEnd, forward, gTankSpread[m_spread], tr );
                g_EntityFuncs.CreateExplosion(tr.vecEndPos, pev.angles, edict(), pev.impulse, true);
                CFuncTank::Fire( barrelEnd, forward, pev );
            }
        }
        else
    ...
    void Fire( const Vector barrelEnd, const Vector forward, entvars_t* pevAttacker )
    {
        if ( m_fireLast != 0 )
        {
            ...
            if ( bulletCount > 0 )
            {
                ...
                {
                    CBaseEntity* pRocket = g_EntityFuncs.CreateRPGRocket(barrelEnd, pev.angles, edict());
                }
                CFuncTank::Fire( barrelEnd, forward, pev );
            }
        }
        ...
        if ( m_fireLast != 0 )
        {
            ...
            if ( bulletCount > 0 )
            {
                ...
                for (int i = 0; i < bulletCount; i++ )
                {
                    ...
                    SET_MODEL(pProj.self, szSprPath);
                }
                CFuncTank::Fire( barrelEnd, forward, pev );
            }
        }
